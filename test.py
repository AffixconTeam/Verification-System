    ...
    try:
        ...
        if data.IDNumber:
            ...
            SELECT
                ...
                END AS dob_score
            FROM
                PUBLIC.{country_sources[data.CountryPrefix]['table_name']}
            )
            SELECT *
        ...
        if 'MOBILE' in df.columns:
            df['MobileMatch'] = True if df['MOBILE'][0] == data.Mobile else False
        if 'EMAIL' in df.columns:
            df['EmailMatch'] = True if df['EMAIL'][0] == data.Email else False
        

        if data.CountryPrefix in ('indonisia','mx'):
        ...
        if data.CountryPrefix not in ('indonisia','mx'):
